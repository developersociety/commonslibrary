...
export class SearchOptionManager extends React.Component {
    ...
    componentDidMount() {
        ...
        if (this.props.preselectedTag !== undefined) {
            fetch(`${'/api/v1/tags' + '?id='}${this.props.preselectedTag}`, {
                ...
                .then((data) => {
                    if (data.length === 1) {
                        const newState = this.state.selectedOptions;
    ...
    handleOptionSelection(option) {
        const prevSelected = this.state.selectedOptions;
        const checkSelected = this.checkSelected(option);
    ...
    render() {
        ...
        return (
            <div className="tag-list">
                ...
                {searchOptionsAvailable && <div className="tag inactive">None selected</div>}
            </div>
        );
    }
}
