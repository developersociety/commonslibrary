...
class ReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Report', {
            'fields': (
                'body', 'get_abused_comment', 'get_comment_url', 'get_resource_url', 'reviewed',
            )
        }),
        (
            'Meta', {
    ...
    def get_resource_url(self, obj):
        return format_html('<a href="{url}" target="_blank">Resource URL</a>'.format(
            url=obj.comment.resource.get_absolute_url(),
        ))
    get_resource_url.short_description = 'Resource URL'

    def get_comment_url(self, obj):
        return format_html('<a href="{url}" target="_blank">Comment URL</a>'.format(
    ...
    def get_queryset(self, request):
        ...
        if not request.user.is_superuser and request.user.approved_organisations.exists():
            qs = qs.filter(comment__resource__organisation__in=request.user.approved_organisations.all()).distinct()
        return qs
